...
import { setOnboardingStep, setSelfReportedLevel } from "@/app/(auth)/actions";
...
export default function OnboardingLevelPage() {
  ...
  async function goTo(nextLevel: string) {
    ...
    await setSelfReportedLevel(nextLevel === "unsure" ? null : nextLevel);
  ...
  return (
    <div>
      <OnboardingStepBar step={1} loading={pending} />
      ...
      <RadioGroup value={level} onValueChange={setLevel} disabled={pending} className="gap-3">
        {LEVELS.map((option) => (
          <ChoiceCard
            key={option.value}
            value={option.value}
      ...
      <button
        ...
        disabled={pending}
        onClick={() => goTo("unsure")}
        className="t-label-sm mt-4 text-fg-link hover:text-fg-link-hover disabled:opacity-50"
      >
        Not sure — take the five-minute check
      </button>

      <Button className="mt-8 w-full" disabled={!level} loading={pending} onClick={() => goTo(level)}>
        Continue
      </Button>
    </div>
  );
